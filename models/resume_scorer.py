...
# Lazy-load the pre-trained model to avoid slow startup
nlp = None
...
def calculate_similarity(text1, text2):
    ...
    # Average pooling to get fixed-size vectors
    vec1_mean = torch.mean(torch.as_tensor(vec1), dim=1)
    vec2_mean = torch.mean(torch.as_tensor(vec2), dim=1)
...
def score_resume(resume_data, job_description_data):
    ...
    entity_score = (
        len(resume_entities & job_entities) / len(job_entities) * 100
        if job_entities else 0
    )

    skills_score = calculate_similarity(
        resume_data['skills'], job_description_data['skills']
    ) * 100

    experience_score = calculate_similarity(
        resume_data['experience'], job_description_data['experience']
    ) * 100

    education_score = calculate_similarity(
        resume_data['education'], job_description_data['education']
    ) * 100
...
def generate_feedback(resume_data, job_description_data):
    ...
    if missing_skills:
        feedback.append(
            f"Your skills section could be improved. Consider including skills such as: "
            f"{', '.join(list(missing_skills)[:10])}."
        )

    if missing_experience:
        feedback.append(
            f"Your experience section could be improved. Consider including experiences such as: "
            f"{', '.join(list(missing_experience)[:10])}."
        )

    if missing_education:
        feedback.append(
            f"Your education section could be improved. Ensure it highlights education such as: "
            f"{', '.join(list(missing_education)[:10])}."
        )
